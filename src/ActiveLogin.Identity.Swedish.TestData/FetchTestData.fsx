...
let getPins url =
    async { let! pins = Pins.AsyncLoad url
...
let parseAsTuple (s:string) =
    (s.[ 0..3 ], s.[ 4..5 ], s.[ 6..7 ], s.[ 8..10 ], s.[ 11.. 11 ])

let tupleAsString (year, month, day, birthNumber, checksum) =
    sprintf "            (%s,%s,%s,%s,%s)" year month day birthNumber checksum 

let appendLine (sb:StringBuilder) line = sb.AppendLine line |> ignore

sb.Clear()
sb.Append(header)
pins
|> Seq.iter (parseAsTuple >> tupleAsString >> appendLine sb)
sb.Append(footer)
