...
nuget FSharp.Data 
nuget ActiveLogin.Identity.Swedish //"
...
let parsePins twelveDigitInt =
    async {
        let twelveDigitStr = string twelveDigitInt
        let tenDigitStr = 
            match SwedishPersonalIdentityNumber.parse  twelveDigitStr with
            | Ok p -> p 
            | Error _ -> failwithf "Could not parse %s" twelveDigitStr
            |> SwedishPersonalIdentityNumber.to10DigitString
        return (twelveDigitStr, tenDigitStr)
    }
...
let writePins (sb:StringBuilder) pins =
    pins
    ||> sprintf "            (\"%s\", \"%s\")" 
    |> sb.AppendLine
    |> ignore
