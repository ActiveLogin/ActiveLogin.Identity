...
module Result =
    let OkValue = function
        | Ok value -> value
        | Result.Error e -> e.ToString() |> sprintf "test setup error: %s" |> failwith
...
type InvalidYearGen() =
    static member Gen() : Arbitrary<InvalidYear> =
        (1, 9999) ||> outsideRange |> Gen.map InvalidYear |> Arb.fromGen
...
type InvalidMonthGen() =
    static member Gen() : Arbitrary<InvalidMonth> =
        (1, 12) ||> outsideRange |> Gen.map InvalidMonth |> Arb.fromGen

type ValidMonth = ValidMonth of int

type ValidMonthGen() =
    static member Gen() : Arbitrary<ValidMonth> =
        (1,12) |> Gen.choose |> Gen.map ValidMonth |> Arb.fromGen
...
type WithInvalidDayGen() =
    static member Gen() : Arbitrary<WithInvalidDay> =
        withInvalidDay |> Arb.fromGen
...
type WithValidDayGen() =
    static member Gen() : Arbitrary<WithValidDay> =
        withValidDay |> Arb.fromGen
...
type TwoEqualPinsGen() =
    static member Gen() : Arbitrary<TwoEqualPins> =
        gen {
            ...
            let pin1 = values |> SwedishPersonalIdentityNumber.create |> Result.OkValue
            let pin2 = values |> SwedishPersonalIdentityNumber.create |> Result.OkValue
...
type ValidPinGen() =
    static member Gen() : Arbitrary<ValidPin> =
        gen {
            let! pin = validValues |> Gen.map (SwedishPersonalIdentityNumber.create >> Result.OkValue)
        }